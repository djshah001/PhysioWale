    ...
    >
          ...
          <View className="flex-row justify-between items-start">
            <View className="flex-row items-center">
              <Avatar.Image
                size={40}
                source={{ uri: review.userId?.profilePic || `https://ui-avatars.com/api/?name=${encodeURIComponent(review.userId?.name || 'User')}&background=random` }}
              />
              <View className="ml-2">
                ...
                <View className="flex-row items-center">
                  ...
                  {review.isVerified && (
                    ...
                    >
                      <Icon source="check-circle" size={10} color={colors.primary[400]} />
                      <Text className="text-xs font-ossemibold text-primary-400 ml-0.5">Verified</Text>
                    </LinearGradient>
                  )}
                </View>
              </View>
            ...
            <LinearGradient
              colors={review.rating >= 4 ? colors.gradients.success : review.rating >= 3 ? colors.gradients.warning : colors.gradients.error}
              start={{ x: 0, y: 0 }}
              end={{ x: 1, y: 0 }}
              className="flex-row items-center px-3 py-1.5 rounded-full"
            >
              <Icon source="star" size={14} color="white" />
              <Text className="font-ossemibold text-sm text-white-400 ml-1">{review.rating}</Text>
            </LinearGradient>
          </View>
              ...
              >
                <View className="p-3">
                  <View className="flex-row items-center mb-2">
                    <Icon source="star-box-multiple" size={14} color={colors.accent[600]} />
                ...
                <View className="p-3">
                  <View className="flex-row items-center mb-2">
                    <View className="bg-secondary-100 rounded-full p-1 mr-2">
                      <Icon source="reply" size={14} color={colors.secondary[400]} />
                    </View>
